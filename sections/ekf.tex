% Section 6 - The Extended Kalman Filter
...
\begin{frame}{Odometric reconstruction}{An elementary sensor fusion technique}
  \only<1->{
    Suppose you have a robot equipped with $m$ \textbg{sensors} producing \textbg{measurements} $q_i$, $i = 1,\dotsc,m$, and a \textbg{motion model} that predicts the \textbg{state} of the robot $x_k$ at time $k$ given the state at time $k-1$ and the control input $u_k$ applied to the robot.\\
    \bigskip
...
\begin{frame}{Modelling noise}{Gaussian random variables}
  ...
  \end{itemize}
  Having distinct sensors makes these variables \textbg{independent}. This is reasonable because:
  \begin{itemize}
...
\begin{frame}{Modelling noise}{Combining variables}
  Similarly, we can combine the two measurements $q_1$ and $q_2$ to obtain a \textbg{better estimate} of $x$, as $x \sim \mathcal{N}(q,\sigma^2)$, where:
  \begin{itemize}
...
\end{frame}
\begin{frame}{Kalman filter}{The scalar case}
  Initialize the \textbg{estimate} as $\hat{x}_0$ and the \textbg{covariance} as $P_0$.\\
...
\begin{frame}{Extended Kalman Filter}{The linearization}
  ...
  Grouping some \textbg{known terms}, these lead to a \textbg{linear system}:
  \begin{subequations}
    \begin{align}
    ...
    \end{align}
  \end{subequations}
  where \eqref{eq:newmeas} can be considered a measurement, subtracting a \textbg{known term} from the \textbg{true measurement}.
\end{frame}
...
% TODO:
% - rearrange section to explain this first
% - it works empyrically. Pointers to other filters and estimators
% - robot_localization
